...
from ClusterShell.NodeSet import set_std_group_resolver, set_std_group_resolver_config
...
class CLINodesetTestBase(unittest.TestCase):
    ...
    def _nodeset_t(
        self, args, stdin, expected_stdout, expected_rc=0, expected_stderr=None
    ):
        CLI_main(
            self,
            main,
...
class CLINodesetTest(CLINodesetTestBase):
    ...
    def _battery_count(self, args):
        ...
        self._nodeset_t(
            args + ["--count", "foo[395-442]", "foo[1-200,245-394]"], None, b"398\n"
        )
        self._nodeset_t(
            args + ["--count", "foo[395-442]", "foo", "foo[1-200,245-394]"],
            None,
            b"399\n",
        )
        self._nodeset_t(
            args + ["--count", "foo[395-442]", "foo", "foo[0-200,245-394]"],
            None,
            b"400\n",
        )
        self._nodeset_t(
            args + ["--count", "foo[395-442]", "bar3,bar24", "foo[1-200,245-394]"],
            None,
            b"400\n",
        )
    ...
    def test_002_count_intersection(self):
        ...
        self._nodeset_t(
            ["--count", "foo", "--intersection", "foo", "-i", "bar"], None, b"0\n"
        )
        self._nodeset_t(["--count", "foo[0]", "--intersection", "foo0"], None, b"1\n")
        self._nodeset_t(["--count", "foo[2]", "--intersection", "foo"], None, b"0\n")
        self._nodeset_t(
            ["--count", "foo[1,2]", "--intersection", "foo[1-2]"], None, b"2\n"
        )
        self._nodeset_t(
            ["--count", "foo[395-442]", "--intersection", "foo[1-200,245-394]"],
            None,
            b"0\n",
        )
        self._nodeset_t(
        ...
        self._nodeset_t(
            ["--count", "foo[395-442]", "-i", "foo", "-i", "foo[0-200,245-394]"],
            None,
            b"0\n",
        )
        self._nodeset_t(
    ...
    def test_003_count_intersection_stdin(self):
        ...
        self._nodeset_t(
            ["--count", "--intersection", "foo", "-i", "bar"], "foo\n", b"0\n"
        )
        ...
        self._nodeset_t(
            ["--count", "--intersection", "foo[1-200,245-394]"],
            "foo[395-442]\n",
            b"0\n",
        )
        self._nodeset_t(
            ["--count", "--intersection", "foo", "-i", "foo[1-200,245-394]"],
            "foo[395-442]\n",
            b"0\n",
        )
        self._nodeset_t(
            ["--count", "-i", "foo", "-i", "foo[0-200,245-394]"],
            "foo[395-442]\n",
            b"0\n",
        )
        self._nodeset_t(
            ["--count", "--intersection", "bar3,bar24", "-i", "foo[1-200,245-394]"],
            "foo[395-442]\n",
            b"0\n",
        )
    ...
    def _battery_fold(self, args):
        ...
        self._nodeset_t(
            args + ["--fold", "foo[1-200,245-394]"], None, b"foo[1-200,245-394]\n"
        )
        self._nodeset_t(
            args + ["--fold", "foo[395-442]", "foo[1-200,245-394]"],
            None,
            b"foo[1-200,245-442]\n",
        )
        self._nodeset_t(
            args + ["--fold", "foo[395-442]", "foo", "foo[1-200,245-394]"],
            None,
            b"foo,foo[1-200,245-442]\n",
        )
        self._nodeset_t(
            args + ["--fold", "foo[395-442]", "foo", "foo[0-200,245-394]"],
            None,
            b"foo,foo[0-200,245-442]\n",
        )
        self._nodeset_t(
            args + ["--fold", "foo[395-442]", "bar3,bar24", "foo[1-200,245-394]"],
            None,
    ...
    def test_005_fold_autostep(self):
        ...
        self._nodeset_t(
            ["--autostep=2", "-f", "foo0", "foo2", "foo4", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=2", "-f", "foo4", "foo2", "foo0", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=3", "-f", "foo0", "foo2", "foo4", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=4", "-f", "foo0", "foo2", "foo4", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=5", "-f", "foo0", "foo2", "foo4", "foo6"],
            None,
            b"foo[0,2,4,6]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-f", "foo0", "foo2", "foo4", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-f", "foo4", "foo2", "foo0", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-f", "foo4", "foo2", "foo0", "foo2", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-f", "foo4", "foo2", "foo0", "foo5", "foo6"],
            None,
            b"foo[0,2,4-6]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-f", "foo4", "foo2", "foo0", "foo9", "foo6"],
            None,
            b"foo[0,2,4,6,9]\n",
        )
        self._nodeset_t(
            ["--autostep=75%", "-f", "foo0", "foo2", "foo4", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=75%", "-f", "foo4", "foo2", "foo0", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=80%", "-f", "foo4", "foo2", "foo0", "foo2", "foo6"],
            None,
            b"foo[0-6/2]\n",
        )
        self._nodeset_t(
            ["--autostep=80%", "-f", "foo4", "foo2", "foo0", "foo5", "foo6"],
            None,
            b"foo[0,2,4-6]\n",
        )
        self._nodeset_t(
            ["--autostep=80%", "-f", "foo4", "foo2", "foo0", "foo9", "foo6"],
            None,
            b"foo[0-6/2,9]\n",
        )
        self._nodeset_t(
            ["--autostep=81%", "-f", "foo4", "foo2", "foo0", "foo9", "foo6"],
            None,
            b"foo[0,2,4,6,9]\n",
        )
        self._nodeset_t(
            ["--autostep=100%", "-f", "foo4", "foo2", "foo0", "foo9", "foo6"],
            None,
            b"foo[0,2,4,6,9]\n",
        )
    ...
    def test_008_expand_separator(self):
        ...
        self._nodeset_t(
            ["--expand", "--separator", ":", "foo", "bar"], None, b"bar:foo\n"
        )
        ...
        self._nodeset_t(["--expand", "-S", " ", "foo[1-2]"], None, b"foo1 foo2\n")
        self._nodeset_t(
            ["--expand", "-S", ",", "foo[1-2],bar"], None, b"bar,foo1,foo2\n"
        )
        self._nodeset_t(
            ["--expand", "-S", "uuu", "foo[1-2],bar"], None, b"baruuufoo1uuufoo2\n"
        )
    ...
    def test_009_fold_xor(self):
        ...
        self._nodeset_t(
            ["--fold", "foo[1-10]", "-X", "foo[5-15]"], None, b"foo[1-4,11-15]\n"
        )
        self._nodeset_t(
            ["--fold", "foo[395-442]", "-X", "foo[1-200,245-394]"],
            None,
            b"foo[1-200,245-442]\n",
        )
        self._nodeset_t(
            ["--fold", "foo[395-442]", "-X", "foo", "-X", "foo[1-200,245-394]"],
            None,
            b"foo,foo[1-200,245-442]\n",
        )
        self._nodeset_t(
            ["--fold", "foo[395-442]", "-X", "foo", "-X", "foo[0-200,245-394]"],
            None,
            b"foo,foo[0-200,245-442]\n",
        )
        self._nodeset_t(
            ["--fold", "foo[395-442]", "-X", "bar3,bar24", "-X", "foo[1-200,245-394]"],
            None,
            b"bar[3,24],foo[1-200,245-442]\n",
        )
        # multiline args (#394)
        self._nodeset_t(
            ["--fold", "foo[1-10]", "-X", "foo5\nfoo6\nfoo7"], None, b"foo[1-4,8-10]\n"
        )
        self._nodeset_t(
            ["--fold", "foo[1-10]", "-X", "foo5\nfoo6\nfoo7", "foo5\nfoo6"],
            None,
    ...
    def test_010_fold_xor_stdin(self):
        ...
        self._nodeset_t(
            ["--fold", "-X", "foo[5-15]"], "foo[1-10]\n", b"foo[1-4,11-15]\n"
        )
        self._nodeset_t(
            ["--fold", "-X", "foo[1-200,245-394]"],
            "foo[395-442]\n",
            b"foo[1-200,245-442]\n",
        )
        self._nodeset_t(
            ["--fold", "-X", "foo", "-X", "foo[1-200,245-394]"],
            "foo[395-442]\n",
            b"foo,foo[1-200,245-442]\n",
        )
        self._nodeset_t(
            ["--fold", "-X", "foo", "-X", "foo[0-200,245-394]"],
            "foo[395-442]\n",
            b"foo,foo[0-200,245-442]\n",
        )
        self._nodeset_t(
            ["--fold", "-X", "bar3,bar24", "-X", "foo[1-200,245-394]"],
            "foo[395-442]\n",
            b"bar[3,24],foo[1-200,245-442]\n",
        )
        # using stdin for -X
        self._nodeset_t(
            ["-f", "foo[2-4]", "-X", "-"], "foo4 foo5 foo6\n", b"foo[2-3,5-6]\n"
        )
        self._nodeset_t(
            ["-f", "-X", "-", "foo[1-6]"],
            "foo4 foo5 foo6\n",
    ...
    def test_011_fold_exclude(self):
        ...
        self._nodeset_t(
            ["--fold", "foo[0-10]", "foo[13-18]", "--exclude", "foo[5-10,15]"],
            None,
    ...
    def test_012_fold_exclude_stdin(self):
        ...
        self._nodeset_t(
            ["--fold", "--exclude", "foo[5-10,15]"],
            "foo[0-10]\nfoo[13-18]\n",
            b"foo[0-4,13-14,16-18]\n",
        )
        # using stdin for -x
        self._nodeset_t(
            ["-f", "foo[1-6]", "-x", "-"], "foo4 foo5 foo6\n", b"foo[1-3]\n"
        )
        self._nodeset_t(
            ["-f", "-x", "-", "foo[1-6]"],
            "foo4 foo5 foo6\n",
    ...
    def test_013_fold_intersection(self):
        ...
        self._nodeset_t(
            ["--fold", "foo[0-10]", "foo[13-18]", "-i", "foo[5-10,15]"],
            None,
            b"foo[5-10,15]\n",
        )
        # numerical bracket folding (#228)
        self._nodeset_t(
            ["--fold", "node123[1-2]", "-i", "node1232"], None, b"node1232\n"
        )
        self._nodeset_t(
            ["--fold", "node023[1-2]0", "-i", "node02320"], None, b"node02320\n"
        )
        self._nodeset_t(
            ["--fold", "node023[1-2]0-ipmi2", "-i", "node02320-ipmi2"],
            None,
            b"node02320-ipmi2\n",
        )
        self._nodeset_t(
    ...
    def test_014_fold_intersection_stdin(self):
        ...
        self._nodeset_t(
            ["--fold", "-i", "node023[1-2]0"], "node02320\n", b"node02320\n"
        )
        self._nodeset_t(
            ["--fold", "-i", "node023[1-2]0-ipmi2"],
            "node02320-ipmi2\n",
            b"node02320-ipmi2\n",
        )
    ...
    def test_017_stdin(self):
        ...
        self._nodeset_t(
            ["--autostep=100%", "-f"], "foo0 foo2 foo4 foo6\n", b"foo[0-6/2]\n"
        )
        self._nodeset_t(
            ["--autostep=0%", "-f"], "foo0 foo2 foo4 foo6\n", b"foo[0-6/2]\n"
        )
    ...
    def test_018_split(self):
        ...
        self._nodeset_t(
            ["--split", "2", "-e", "foo", "bar", "bur", "oof", "gcc"],
            None,
            b"bar bur foo\ngcc oof\n",
        )
        self._nodeset_t(["--split=2", "-f", "foo[2-9]"], None, b"foo[2-5]\nfoo[6-9]\n")
        self._nodeset_t(
            ["--split=2", "-f", "foo[2-3,7]", "bar9"], None, b"bar9,foo2\nfoo[3,7]\n"
        )
        self._nodeset_t(
            ["--split=3", "-f", "foo[2-9]"], None, b"foo[2-4]\nfoo[5-7]\nfoo[8-9]\n"
        )
    ...
    def test_019_contiguous(self):
        ...
        self._nodeset_t(
            ["--contiguous", "-f", "foo", "bar", "bur", "oof", "gcc"],
            None,
            b"bar\nbur\nfoo\ngcc\noof\n",
        )
        self._nodeset_t(
            ["--contiguous", "-e", "foo", "bar", "bur", "oof", "gcc"],
            None,
            b"bar\nbur\nfoo\ngcc\noof\n",
        )
        ...
        self._nodeset_t(
            ["--contiguous", "-f", "foo[2-3,7]", "bar9"],
            None,
            b"bar9\nfoo[2-3]\nfoo7\n",
        )
        self._nodeset_t(
            ["--contiguous", "-R", "-f", "2-3,7", "9"], None, b"2-3\n7\n9\n"
        )
    ...
    def test_020_slice(self):
        ...
        self._nodeset_t(
            ["-I 0-3", "-f", "bar[34-68,89-90]", "-x", "bar34"], None, b"bar[35-38]\n"
        )
        self._nodeset_t(
            ["-I 0-3", "-f", "bar[34-68,89-90]", "-x", "bar35"],
            None,
            b"bar[34,36-38]\n",
        )
        self._nodeset_t(
            ["-I 0-3", "-e", "bar[34-68,89-90]"], None, b"bar34 bar35 bar36 bar37\n"
        )
        self._nodeset_t(["-I 3,1,0,2", "-f", "bar[34-68,89-90]"], None, b"bar[34-37]\n")
        self._nodeset_t(
            ["-I 1,3,7,10,16,20,30,34-35,37", "-f", "bar[34-68,89-90]"],
            None,
            b"bar[35,37,41,44,50,54,64,68,89]\n",
        )
        self._nodeset_t(["-I 8", "-f", "bar[34-68,89-90]"], None, b"bar42\n")
        self._nodeset_t(
            ["-I 8-100", "-f", "bar[34-68,89-90]"], None, b"bar[42-68,89-90]\n"
        )
        self._nodeset_t(
            ["-I 0-100", "-f", "bar[34-68,89-90]"], None, b"bar[34-68,89-90]\n"
        )
        self._nodeset_t(
            ["-I 8-100/2", "-f", "bar[34-68,89-90]"],
            None,
            b"bar[42,44,46,48,50,52,54,56,58,60,62,64,66,68,90]\n",
        )
        self._nodeset_t(
            ["--autostep=2", "-I 8-100/2", "-f", "bar[34-68,89-90]"],
            None,
            b"bar[42-68/2,90]\n",
        )
        self._nodeset_t(
            ["--autostep=93%", "-I 8-100/2", "-f", "bar[34-68,89-90]"],
            None,
            b"bar[42-68/2,90]\n",
        )
        self._nodeset_t(
            ["--autostep=94%", "-I 8-100/2", "-f", "bar[34-68,89-90]"],
            None,
            b"bar[42,44,46,48,50,52,54,56,58,60,62,64,66,68,90]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-I 8-100/2", "-f", "bar[34-68,89-90]"],
            None,
            b"bar[42,44,46,48,50,52,54,56,58,60,62,64,66,68,90]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-I 8-100/2", "-f", "bar[34-68]"],
            None,
            b"bar[42-68/2]\n",
        )
        self._nodeset_t(
            ["--autostep=100%", "-I 8-100/2", "-f", "bar[34-68]"],
            None,
            b"bar[42-68/2]\n",
        )
    ...
    def test_021_slice_stdin(self):
        ...
        self._nodeset_t(
            ["-I 0-3", "-f", "-x", "bar34"], "bar[34-68,89-90]\n", b"bar[35-38]\n"
        )
        self._nodeset_t(
            ["-I 0-3", "-f", "-x", "bar35"], "bar[34-68,89-90]\n", b"bar[34,36-38]\n"
        )
        self._nodeset_t(
            ["-I 0-3", "-e"], "bar[34-68,89-90]\n", b"bar34 bar35 bar36 bar37\n"
        )
        self._nodeset_t(["-I 3,1,0,2", "-f"], "bar[34-68,89-90]\n", b"bar[34-37]\n")
        self._nodeset_t(
            ["-I 1,3,7,10,16,20,30,34-35,37", "-f"],
            "bar[34-68,89-90]\n",
            b"bar[35,37,41,44,50,54,64,68,89]\n",
        )
        ...
        self._nodeset_t(
            ["--autostep=2", "-I 8-100/2", "-f"],
            "bar[34-68,89-90]\n",
            b"bar[42-68/2,90]\n",
        )
        self._nodeset_t(
            ["--autostep=93%", "-I 8-100/2", "-f"],
            "bar[34-68,89-90]\n",
            b"bar[42-68/2,90]\n",
        )
        self._nodeset_t(
            ["--autostep=93.33%", "-I 8-100/2", "-f"],
            "bar[34-68,89-90]\n",
            b"bar[42-68/2,90]\n",
        )
        self._nodeset_t(
            ["--autostep=94%", "-I 8-100/2", "-f"],
            "bar[34-68,89-90]\n",
            b"bar[42,44,46,48,50,52,54,56,58,60,62,64,66,68,90]\n",
        )
        self._nodeset_t(
            ["--autostep=auto", "-I 8-100/2", "-f"],
            "bar[34-68,89-90]\n",
            b"bar[42,44,46,48,50,52,54,56,58,60,62,64,66,68,90]\n",
        )
        self._nodeset_t(
            ["--autostep=2", "-I 8-100/2", "-f"], "bar[34-68]\n", b"bar[42-68/2]\n"
        )

    def test_022_output_format(self):
        """test nodeset -O"""
        self._nodeset_t(
            ["--expand", "--output-format", "/path/%s/", "foo"], None, b"/path/foo/\n"
        )
        self._nodeset_t(
            ["--expand", "-O", "/path/%s/", "-S", ":", "foo"], None, b"/path/foo/\n"
        )
        self._nodeset_t(
            ["--expand", "-O", "/path/%s/", "foo[2]"], None, b"/path/foo2/\n"
        )
        self._nodeset_t(
            ["--expand", "-O", "%s-ib0", "foo[1-4]"],
            None,
            b"foo1-ib0 foo2-ib0 foo3-ib0 foo4-ib0\n",
        )
        self._nodeset_t(
            ["--expand", "-O", "%s-ib0", "-S", ":", "foo[1-4]"],
            None,
            b"foo1-ib0:foo2-ib0:foo3-ib0:foo4-ib0\n",
        )
        self._nodeset_t(
            ["--fold", "-O", "%s-ipmi", "foo", "bar"], None, b"bar-ipmi,foo-ipmi\n"
        )
        self._nodeset_t(
            ["--fold", "-O", "%s-ib0", "foo1", "foo2"], None, b"foo[1-2]-ib0\n"
        )
        self._nodeset_t(
            ["--fold", "-O", "%s-ib0", "foo1", "foo2", "bar1", "bar2"],
            None,
            b"bar[1-2]-ib0,foo[1-2]-ib0\n",
        )
        self._nodeset_t(
            ["--fold", "-O", "%s-ib0", "--autostep=auto", "foo[1-9/2]"],
            None,
            b"foo[1-9/2]-ib0\n",
        )
        self._nodeset_t(
            ["--fold", "-O", "%s-ib0", "--autostep=6", "foo[1-9/2]"],
            None,
            b"foo[1,3,5,7,9]-ib0\n",
        )
        self._nodeset_t(
            ["--fold", "-O", "%s-ib0", "--autostep=5", "foo[1-9/2]"],
            None,
            b"foo[1-9/2]-ib0\n",
        )
        self._nodeset_t(
            ["--count", "-O", "result-%s", "foo1", "foo2"], None, b"result-2\n"
        )
        self._nodeset_t(
            ["--contiguous", "-O", "%s-ipmi", "-f", "foo[2-3,7]", "bar9"],
            None,
            b"bar9-ipmi\nfoo[2-3]-ipmi\nfoo7-ipmi\n",
        )
        self._nodeset_t(
            ["--split=2", "-O", "%s-ib", "-e", "foo[2-9]"],
            None,
            b"foo2-ib foo3-ib foo4-ib foo5-ib\nfoo6-ib foo7-ib foo8-ib foo9-ib\n",
        )
        self._nodeset_t(
            ["--split=3", "-O", "hwm-%s", "-f", "foo[2-9]"],
            None,
            b"hwm-foo[2-4]\nhwm-foo[5-7]\nhwm-foo[8-9]\n",
        )
        self._nodeset_t(
            ["-I0", "-O", "{%s}", "-f", "bar[34-68,89-90]"], None, b"{bar34}\n"
        )
        # RangeSet mode (-R)
        self._nodeset_t(["--fold", "-O", "{%s}", "--rangeset", "1,2"], None, b"{1-2}\n")
        self._nodeset_t(["--expand", "-O", "{%s}", "-R", "1-2"], None, b"{1} {2}\n")
        self._nodeset_t(
            ["--fold", "-O", "{%s}", "-R", "1-2", "-X", "2-3"], None, b"{1,3}\n"
        )
        self._nodeset_t(
            ["--fold", "-O", "{%s}", "-S", ":", "--rangeset", "1,2"], None, b"{1-2}\n"
        )
        self._nodeset_t(
            ["--expand", "-O", "{%s}", "-S", ":", "-R", "1-2"], None, b"{1}:{2}\n"
        )
        self._nodeset_t(
            ["--fold", "-O", "{%s}", "-S", ":", "-R", "1-2", "-X", "2-3"],
            None,
            b"{1,3}\n",
        )
        self._nodeset_t(
            ["-R", "-I0", "-O", "{%s}", "-f", "34-68,89-90"], None, b"{34}\n"
        )
    ...
    def test_023_axis(self):
        ...
        self._nodeset_t(
            ["--axis=1", "-f"] + ndnodes, None, b"node[0-499]-ib0,node[0-499]-ib1\n"
        )
        ...
        self._nodeset_t(["--axis=2", "-f"] + ndnodes, None, exp_result_str.encode())
    ...
    def test_025_pick(self):
        ...
        for num in range(1, 100):
            self._nodeset_t(
                ["--count", "--pick", str(num), "foo[1-100]"],
                None,
                str(num).encode() + b"\n",
            )
            self._nodeset_t(
                ["--count", "--pick", str(num), "-R", "1-100"],
                None,
                str(num).encode() + b"\n",
            )
...
class CLINodesetGroupResolverTest1(CLINodesetTestBase):
    ...
    def test_022_list(self):
        ...
        self._nodeset_t(
            ["-l", "example[4,95]", "example5"], None, b"@bar\n@foo\n@moo\n"
        )
        self._nodeset_t(
            ["-ll", "example[4,95]", "example5"],
            None,
            b"@bar example[4-5,95]\n@foo example[4-5,95]\n@moo example[4-5,95]\n",
        )
        self._nodeset_t(
            ["-lll", "example[4,95]", "example5"],
            None,
            b"@bar example[4-5,95] 3/100\n@foo example[4-5,95] 3/100\n@moo example[4-5,95] 3/100\n",
        )
        # test empty result
        ...
        self._nodeset_t(
            ["-l", "-X example3"], None, b"@bar\n@foo\n@moo\n"
        )  # no -a, xor from nothing
        self._nodeset_t(
            ["-l", "-", "-i example3"], "example[3,500]\n", b"@bar\n@foo\n@moo\n"
        )
...
class CLINodesetGroupResolverTest2(CLINodesetTestBase):
    ...
    def test_024_groups(self):
        self._nodeset_t(
            ["--split=2", "-r", "unknown2", "unknown3"], None, b"unknown2\nunknown3\n"
        )
        ...
        self._nodeset_t(
            ["-e", "-a"],
            None,
            " ".join(["example%d" % i for i in range(1, 101)]).encode() + b"\n",
        )
    ...
    def test_028_groups(self):
        self._nodeset_t(
            ["-s", "test", "--groupsources"], None, b"test (default)\nother\n"
        )
    ...
    def test_035_groups(self):
        self._nodeset_t(
            ["-s", "other", "--groupsources"], None, b"other (default)\ntest\n"
        )
    ...
    def test_038_groups_output_format(self):
        self._nodeset_t(
            ["-O", "{%s}", "-s", "other", "-r", "-a"], None, b"{@other:baz}\n"
        )
    ...
    def test_040_wildcards(self):
        ...
        self._nodeset_t(
            ["-s", "other", "-f", "nova?[12-42]"],
            None,
            b"nova[030-042,112-142,212-242,312-342,412-442]\n",
        )
        self._nodeset_t(
            ["-s", "other", "-f", "*!*[033]"], None, b"nova[030-032,034-489]\n"
        )
        self._nodeset_t(
            ["-s", "other", "--autostep=3", "-f", "*!*[033-099/2]"],
            None,
            b"nova[030-032,034-100/2,101-489]\n",
        )

    def test_041_completion(self):
        """test nodeset --completion"""
        self._nodeset_t(["--completion"], None, b"@test:\n@other:\n@bar\n@foo\n@moo\n")
        self._nodeset_t(
            ["--completion", "node1", "node2"],
            None,
            b"@test:\n@other:\n@bar\n@foo\n@moo\nnode1 node2\n",
        )
        self._nodeset_t(
        ...
        self._nodeset_t(
            ["-s", "other", "--completion", "node1", "node2"],
            None,
            b"@other:baz\n@other:norf\n@other:qux\nnode1 node2\n",
        )
...
class CLINodesetGroupsConfOption(CLINodesetTestBase):
    ...
    def test_groupsconf_option(self):
        ...
        self._nodeset_t(
            ["--groupsconf", self.custf.name, "--list-all"],
            None,
            b"@artemis\n@selene\n",
        )
        self._nodeset_t(
            ["--groupsconf", self.custf.name, "-f", "@artemis"], None, b"custom[7-42]\n"
        )
