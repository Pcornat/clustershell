...
class NodeSetGroupTest(unittest.TestCase):
    ...
    def testGroupResolverSimple(self):
        ...
        nodeset = NodeSet(
            "montana3,@gpuchassis!montana39,montana77^montana38", resolver=res
        )
    ...
    def testAllNoResolver(self):
        ...
        self.assertRaises(
            NodeSetExternalError, NodeSet.fromall, resolver=RESOLVER_NOGROUP
        )
    ...
    def testConfigListAllNDWithoutAll(self):
        ...
        self.assertEqual(
            grouplist(resolver=res), ["x1y1", "x1y2", "x1y3", "x1y4"]
        )  # cleaned
        # test "@@" group name set
        nodeset = NodeSet("@@", resolver=res)
    ...
    def testConfigGroupsMultipleDirs(self):
        ...
        try:
            ...
            self.assertEqual(
                str(NodeSet("@local1:bar", resolver=res)), "loc1node[1-100]"
            )
            # local2
    ...
    def testGroupResolverND(self):
        ...
        self.assertEqual(
            sorted(nodeset.groups(groupsource="simple").keys()),
            [
                "@simple:rack-all",
                "@simple:rack-x1",
                "@simple:rack-x1y1",
    ...
    def testConfigCFGDIR(self):
        ...
        self.assertEqual(str(NodeSet("@%s" % tmpgroup, resolver=res)), "example[1-100]")
    ...
    def test_fromall_grouplist(self):
        ...
        source = UpcallGroupSource(
            ...
            None,
            r"sed -n 's/^\([0-9A-Za-z_\%%-]*\):.*/\1/p' %s" % test_groups2.name,
            None,
        )
        res = GroupResolver(source)
    ...
    def test_nodeset_wildcard_support_ranges(self):
        ...
        nodeset = NodeSet("foo*", resolver=res)
        self.assertEqual(str(nodeset), "foo[1-3],foo[1-2]-ib[0-1],foobar,foobar1")
        ...
        self.assertEqual(str(nodeset), "bar10,foo[1-2],foo[1-2]-ib[0-1],foobar1")
...
class StaticGroupSource(UpcallGroupSource):
    ...
    def __init__(self, name, data):
        ...
        UpcallGroupSource.__init__(
            self, name, "fake_map", all_upcall, list_upcall, reverse_upcall
        )
        self._data = data
...
class GroupSourceCacheTest(unittest.TestCase):
    ...
    def test_expired_cache_reverse(self):
        ...
        source = StaticGroupSource(
            "cache",
            {"map": {"a": "foo1", "b": "foo2"}, "reverse": {"foo1": "a", "foo2": "b"}},
        )
        source.cache_time = 0.2
        res = GroupResolver(source)
...
class GroupSourceTest(unittest.TestCase):
    ...
    def test_base_class1(self):
        ...
        gs = GroupSource(
            "testsrc", {"gr1": ["n1", "n4", "n3", "n2"], "gr2": ["n9", "n4"]}
        )
    ...
    def test_base_class2(self):
        ...
        gs = GroupSource(
            "testsrc", {"gr1": ["n1", "n4", "n3", "n2"], "gr2": ["n9", "n4"]}, "n[1-9]"
        )
...
class YAMLGroupLoaderTest(unittest.TestCase):
    ...
    def test_one_source(self):
        ...
        self.assertEqual(loader.groups("vendors"), {"apricot": "node"})
    ...
    def test_multi_sources(self):
        ...
        self.assertEqual(loader.groups("vendors"), {"apricot": "node"})
        self.assertEqual(loader.groups("customers"), {"cherry": "client-4-2"})
    ...
    def test_reload(self):
        ...
        self.assertEqual(
            loader.groups("customers"), {"cherry": "client-4-2", "nut": "node42"}
        )
    ...
    def test_numeric_sources(self):
        ...
        sources = list(loader)
        self.assertEqual(len(sources), 1)
        self.assertEqual(loader.groups("111"), {"compute": "sgisummit-rcf-111-[08,10]"})
        # bad
        f = make_temp_file(b"111: { compute: 'sgisummit-rcf-111-[08,10]' }")
        self.assertRaises(GroupResolverConfigError, YAMLGroupLoader, f.name)
    ...
    def test_numeric_group(self):
        ...
        sources = list(loader)
        self.assertEqual(len(sources), 1)
        self.assertEqual(loader.groups("courses"), {"101": "workstation-[1-10]"})
        # bad
        f = make_temp_file(b"courses: { 101: 'workstation-[1-10]' }")
        self.assertRaises(GroupResolverConfigError, YAMLGroupLoader, f.name)
    ...
    def test_list_group(self):
        ...
        self.assertEqual(
            resolver.group_nodes("bubba"), ["pickup-1,pickup-2,tractor-[1-2]"]
        )
...
class GroupResolverYAMLTest(unittest.TestCase):
    ...
    def test_yaml_permission_denied(self):
        ...
        yamlfile1 = make_temp_file(
            b'yaml1: {foo: "example[1-4]"}', suffix=".yaml", dir=tdir.name
        )
        yamlfile2 = make_temp_file(
            b'yaml2: {bar: "example[5-8]"}', suffix=".yaml", dir=tdir.name
        )
        ...
        try:
            ...
            self.assertRaises(
                GroupResolverSourceError, NodeSet, "@yaml2:bar", resolver=res
            )
        finally:
            yamlfile1.close()
            yamlfile2.close()
