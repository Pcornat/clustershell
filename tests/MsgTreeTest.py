...
class MsgTreeTest(unittest.TestCase):
    ...
    def test_003_iterators(self):
        ...
        cnt = 0
        for msg, keys in tree.walk(
            match=lambda k: k[1] == "newkey", mapper=itemgetter(0)
        ):
            cnt += 1
        ...
        cnt = 0
        for msg, keys in tree.walk(match=lambda k: k[1] == "key", mapper=itemgetter(0)):
            cnt += 1
    ...
    def test_005_remove(self):
        ...
        self.assertEqual(len(tree), 3)
        for msg, keys in tree.walk(match=lambda k: k[0] == "w1", mapper=itemgetter(1)):
    ...
    def test_008_trace_mode(self):
        ...
        result = [
            (m, sorted(k), d, c) for m, k, d, c in sorted(list(tree.walk_trace()))
        ]
        self.assertEqual(
            result,
            [
    ...
    def test_009_defer_to_shift_mode(self):
        ...
        self.assertEqual(
            sorted([(k, e.message()) for e, k in tree.walk()]),
            [
                (["item1"], b"message0\nmessage3"),
                (["item2"], b"message1\nmessage4"),
                (["item3"], b"message2"),
