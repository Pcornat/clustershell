...
class CLIClushTest_A(unittest.TestCase):
    ...
    def test_000_display(self):
        ...
        self._clush_t(["-N", "-w", HOSTNAME, "echo", "ok", "ok"], None, b"ok ok\n")
        self._clush_t(
            ["-w", "badhost,%s" % HOSTNAME, "-x", "badhost", "echo", "ok"],
            None,
            self.output_ok,
        )
        ...
        self._clush_t(["--nostdin", "-w", HOSTNAME, "echo", "ok"], None, self.output_ok)

        self._clush_t(
            ["-w", HOSTNAME, "--color=always", "echo", "ok"], None, self.output_ok_color
        )
        self._clush_t(
            ["-w", HOSTNAME, "--color=never", "echo", "ok"], None, self.output_ok
        )

        # issue #352
        self._clush_t(
            ["-N", "-R", "exec", "-w", "foo[1-2]", "-b", "echo", "test"],
            None,
            b"test\n",
        )
    ...
    def test_002_fanout(self):
        ...
        self._clush_t(["-f", "1", "-w", HOSTNAME, "echo", "ok"], None, self.output_ok)
    ...
    def test_004_ssh_options(self):
        ...
        self._clush_t(
            ["-o", "-oStrictHostKeyChecking=no", "-w", HOSTNAME, "echo", "ok"],
            None,
            self.output_ok,
        )
        self._clush_t(
            [
                "-o",
                "-oStrictHostKeyChecking=no -oForwardX11=no",
                "-w",
                HOSTNAME,
                "echo",
                "ok",
        ...
        self._clush_t(["-u", "30", "-w", HOSTNAME, "echo", "ok"], None, self.output_ok)
        self._clush_t(
            ["-t", "30", "-u", "30", "-w", HOSTNAME, "echo", "ok"], None, self.output_ok
        )
    ...
    def test_006_output_gathering(self):
        ...
        self._clush_t(["-w", HOSTNAME, "-bL", "echo", "ok"], None, self.output_ok)
        self._clush_t(["-w", HOSTNAME, "-qbL", "echo", "ok"], None, self.output_ok)
        self._clush_t(["-w", HOSTNAME, "-BL", "echo", "ok"], None, self.output_ok)
        self._clush_t(["-w", HOSTNAME, "-qBL", "echo", "ok"], None, self.output_ok)
        self._clush_t(["-w", HOSTNAME, "-BLS", "echo", "ok"], None, self.output_ok)
        self._clush_t(["-w", HOSTNAME, "-qBLS", "echo", "ok"], None, self.output_ok)

        s = "%s: ok\n---------------\n%s\n---------------\nok\n" % (HOSTNAME, HOSTNAME)
        self._clush_t(["-w", HOSTNAME, "-vb", "echo", "ok"], None, s.encode())
    ...
    def test_008_file_copy(self):
        ...
        self._clush_t(["-w", HOSTNAME, "-c", sf.name, "--dest", f2.name], None, b"")
        f2.seek(0)
        ...
        try:
            f2 = tempfile.NamedTemporaryFile()
            self._clush_t(
                ["-w", HOSTNAME, "-c", sf.name, sf2.name, "--dest", tdir.name],
                None,
                b"",
            )
        ...
        try:
            f2 = tempfile.NamedTemporaryFile()
            self._clush_t(["-w", HOSTNAME, "-c", sf.name, sf2.name], None, b"")
            sf.seek(0)
            sf2.seek(0)
            self.assertEqual(sf.read(), content)
        ...
        f2.seek(0)
        self.assertEqual(open("%s.%s" % (sf.name, HOSTNAME), "rb").read(), content)
    ...
    def test_010_diff(self):
        ...
        self._clush_t(
            ["-w", "%s,localhost" % HOSTNAME, "--diff", "echo", "ok"], None, b""
        )
    ...
    def test_013_stdin(self):
        ...
        self._clush_t(
            ["-w", HOSTNAME, "gzip -d"],
            codecs.decode(
                b"1f8b0800869a744f00034bcbcf57484a2ce2020027b4dd1308000000", "hex"
            ),
            s.encode(),
        )
    ...
    def test_015_stderr(self):
        """test clush (stderr)"""
        s = "%s: err\n" % HOSTNAME
        self._clush_t(["-w", HOSTNAME, "echo err 1>&2"], None, b"", 0, s.encode())
        self._clush_t(
            ["-b", "-w", HOSTNAME, "-q", "echo err 1>&2"], None, b"", 0, s.encode()
        )
        s = "---------------\n%s\n---------------\nerr\n" % HOSTNAME
        self._clush_t(["-B", "-w", HOSTNAME, "-q", "echo err 1>&2"], None, s.encode())
    ...
    def test_017_retcodes(self):
        ...
        self._clush_t(
            ["-O", "maxrc=yes", "-w", HOSTNAME, "/bin/false"], None, b"", 1, exp_err
        )
        # -O takes precedence over --maxrc
        self._clush_t(
            ["--maxrc", "-O", "maxrc=no", "-w", HOSTNAME, "/bin/false"],
            None,
            b"",
            0,
            exp_err,
        ...
        self._clush_t(
            ["-O", "maxrc=no", "--maxrc", "-w", HOSTNAME, "/bin/false"],
            None,
            b"",
            0,
            exp_err,
        ...
        for i in (1, 2, 127, 128, 255):
            ...
            self._clush_t(
                ["-S", "-w", HOSTNAME, "exit %d" % i], None, b"", i, s.encode()
            )
        self._clush_t(["-v", "-w", HOSTNAME, "/bin/false"], None, b"", 0, exp_err)

        duo = str(NodeSet("%s,localhost" % HOSTNAME))
        s = "clush: %s (%d): exited with exit code 1\n" % (duo, 2)
        self._clush_t(["-w", duo, "-b", "/bin/false"], None, b"", 0, s.encode())
        s = "clush: %s: exited with exit code 1\n" % duo
        self._clush_t(["-w", duo, "-b", "-q", "/bin/false"], None, b"", 0, s.encode())
        s = "clush: %s (%d): exited with exit code 1\n" % (duo, 2)
        self._clush_t(["-w", duo, "-S", "-b", "/bin/false"], None, b"", 1, s.encode())
    ...
    def test_019_timeout(self):
        ...
        self._clush_t(["-w", HOSTNAME, "-u", "1", "sleep 3"], None, b"", 0, s.encode())
        self._clush_t(
            ["-w", HOSTNAME, "-u", "1", "-b", "sleep 3"], None, b"", 0, s.encode()
        )
    ...
    def test_021_file_copy_timeout(self):
        ...
        s = "clush: %s: command timeout\n" % HOSTNAME
        self._clush_t(
            ["-w", HOSTNAME, "-u", "0.01", "-c", f.name], None, b"", 0, s.encode()
        )
    ...
    def test_025_worker(self):
        """test clush (worker)"""
        self._clush_t(
            ["-w", HOSTNAME, "--worker=ssh", "echo ok"], None, self.output_ok, 0
        )
        self._clush_t(["-w", HOSTNAME, "-R", "ssh", "echo ok"], None, self.output_ok, 0)
        ...
        self._clush_t(
            ["-w", HOSTNAME, "--worker=exec", "-d", "echo ok"],
            None,
            re.compile(rxs.encode()),
            0,
        )
        self._clush_t(
            ["-w", HOSTNAME, "-R", "exec", "-d", "echo ok"],
            None,
            re.compile(rxs.encode()),
            0,
        )
    ...
    def test_026_keyboard_interrupt(self):
        ...
        process = Popen(
            [python_exec, "-m", "ClusterShell.CLI.Clush"] + args,
            stderr=PIPE,
            stdout=PIPE,
            bufsize=0,
        )
    ...
    def test_027_warn_shell_globbing_nodes(self):
        ...
        try:
            os.chdir(tdir.name)
            s = (
                b"Warning: using '-w %s' and local path '%s' exists, was it "
    ...
    def test_028_warn_shell_globbing_exclude(self):
        ...
        try:
            os.chdir(tdir.name)
            rxs = (
                r"^Warning: using '-x %s' and local path "
                r"'%s' exists, was it expanded by the shell\?\n" % (HOSTNAME, HOSTNAME)
            )
            self._clush_t(
                ["-S", "-w", "badhost,%s" % HOSTNAME, "-x", HOSTNAME, "echo", "ok"],
                None,
                b"",
                255,
                re.compile(rxs.encode()),
    ...
    def test_029_hostfile(self):
        ...
        self._clush_t(["--hostfile", f.name, "echo", "ok"], None, self.output_ok)
        f2 = make_temp_file(HOSTNAME.encode())
        self._clush_t(
            ["--hostfile", f.name, "--hostfile", f2.name, "echo", "ok"],
            None,
            self.output_ok,
        )
        self.assertRaises(
    ...
    def test_030_config_options(self):
        """test clush -O/--option"""
        self._clush_t(
            ["--option", "color=never", "-w", HOSTNAME, "echo", "ok"],
            None,
            self.output_ok,
        )
        self._clush_t(
            ["--option", "color=always", "-w", HOSTNAME, "echo", "ok"],
            None,
            self.output_ok_color,
        )
        self._clush_t(
            ["--option=color=never", "-w", HOSTNAME, "echo", "ok"], None, self.output_ok
        )
        self._clush_t(
            ["--option=color=always", "-w", HOSTNAME, "echo", "ok"],
            None,
    ...
    def test_031_progress(self):
        ...
        self._clush_t(
            ["-w", HOSTNAME, "--progress", "echo", "ok"], None, self.output_ok
        )
        self._clush_t(
            ["-w", HOSTNAME, "--progress", "sleep", "2"],
            None,
            b"",
            0,
        ...
        self._clush_t(
            ["-w", "%s,localhost" % HOSTNAME, "--progress", "sleep", "2"],
            b"AAAAAAAAAAAAAA",
            b"",
            0,
        ...
        self._clush_t(["-w", HOSTNAME, "--progress", "-q", "sleep", "2"], None, b"", 0)
        self._clush_t(
            ["-w", HOSTNAME, "-b", "--progress", "-q", "sleep", "2"], None, b"", 0, b""
        )
        self._clush_t(
            ["-w", HOSTNAME, "-b", "--progress", "-q", "sleep", "2"],
            b"AAAAAAAAAAAAAAAA",
            b"",
            0,
            b"",
        ...
        self._clush_t(
            ["-w", HOSTNAME, "--progress", "echo foo >&2; echo bar; sleep 2"],
            None,
            s.encode(),
            0,
            re.compile(err_rxs.encode()),
        )
    ...
    def test_032_worker_pdsh(self):
        ...
        # support write
        self._clush_t(
            ["-w", HOSTNAME, "--worker=pdsh", "--nostdin", "echo ok"],
            None,
            self.output_ok,
            0,
        )
    ...
    def test_033_worker_pdsh_tty(self):
        ...
        try:
            self._clush_t(
                ["-w", HOSTNAME, "--worker=pdsh", "echo ok"], None, self.output_ok, 0
            )
        finally:
            delattr(ClusterShell.CLI.Clush, "_f_user_interaction")
    ...
    def test_034_pick(self):
        ...
        self._clush_t(
            ["-w", "%s,localhost" % HOSTNAME, "--pick", "1", "echo foo"],
            None,
            re.compile(rxs.encode()),
        )
        rxs = r"^((localhost|%s): foo\n){2}$" % HOSTNAME
        self._clush_t(
            ["-w", "%s,localhost" % HOSTNAME, "--pick", "2", "echo foo"],
            None,
            re.compile(rxs.encode()),
        )
    ...
    def test_035_sorted_line_mode(self):
        ...
        self._clush_t(["-w", HOSTNAME, "-L", "echo", "ok"], None, self.output_ok)

        # Issue #326
        cmd = (
            "bash -c 's=%h; n=${s//[!0-9]/}; if [[ $(expr $n %% 2) == 0 ]]; then "
            "echo foo; else echo bar; fi'"
        )

        self._clush_t(
            ["-w", "cs[01-03]", "--worker=exec", "-L", cmd],
            None,
            b"cs01: bar\ncs02: foo\ncs03: bar\n",
            0,
        )
    ...
    def test_036_sorted_gather(self):
        ...
        cmd = (
            "bash -c 's=%h; n=${s//[!0-9]/}; if [[ $(expr $n %% 2) == 0 ]];"
            "then echo foo; else echo bar; fi'"
        )

        self._clush_t(
            ["-w", "cs[01-03]", "--worker=exec", "-b", cmd],
            None,
            b"---------------\ncs[01,03] (2)\n---------------\nbar\n"
            b"---------------\ncs02\n---------------\nfoo\n",
            0,
    ...
    def test_039_conf_option(self):
        ...
        self._clush_t(
            ["-b", "-R", "exec", "-w", "foo[1-10]", "echo ok"],
            b"",
            b"---------------\nfoo[1-10] (10)\n---------------\nok\n",
        )

        self._clush_t(
            ["--conf", custf.name, "-b", "-R", "exec", "-w", "foo[1-10]", "echo ok"],
            b"",
            b"---------------\nfoo[1-10]\n---------------\nok\n",
        )
    ...
    def test_041_outdir_errdir(self):
        ...
        try:
            self._clush_t(
                ["-w", HOSTNAME, "--outdir", odir.name, "echo", "ok"],
                None,
                self.output_ok,
            )
        ...
        try:
            self._clush_t(
                ["-w", HOSTNAME, "--errdir", edir.name, "echo", "ok", ">&2"],
                None,
                None,
                0,
                self.output_ok,
    ...
    def test_042_command_prefix(self):
        ...
        s = "%s: foobar\n" % HOSTNAME
        self._clush_t(
            ["-O", "command_prefix=echo", "-w", HOSTNAME, "foobar"], None, s.encode()
        )
        self._clush_t(
            ["-O", "command_prefix=echo", "--nostdin", "-w", HOSTNAME, "foobar"],
            None,
            s.encode(),
        )
    ...
    def test_043_password_prompt(self):
        ...
        try:
            ...
            expected = s.encode()
            self._clush_t(
                ["-O", "password_prompt=yes", "-w", HOSTNAME, "cat"], None, expected
            )
            self._clush_t(
                ["-O", "password_prompt=yes", "-w", HOSTNAME, "cat"],
                b"test\n",
                expected + ("%s: test\n" % HOSTNAME).encode(),
            )
            self._clush_t(
                ["-O", "password_prompt=yes", "--nostdin", "-w", HOSTNAME, "cat"],
                None,
                expected,
            )
            self._clush_t(
...
class CLIClushTest_D_StdinFIFO(unittest.TestCase):
    ...
    def test_300_fifo_stdin(self):
        ...
        s = "%s: 0123456789\n" % HOSTNAME
        self._clush_t(["-w", HOSTNAME, "-v", "cat"], None, s.encode(), 0, b"")

    def test_301_fifo_stdin(self):
        """test clush with stdin is a fifo (not read)"""
        s = "%s: ok\n" % HOSTNAME
        self._clush_t(["-w", HOSTNAME, "-v", "echo ok"], None, s.encode(), 0, b"")
...
class CLIClushTest_E_Topology(unittest.TestCase):
    ...
    def setUp(self):
        self.topofile = make_temp_file(
            dedent(
                """
                        [Main]
                        %s: localhost
                        localhost: remote-node"""
                % HOSTNAME
            ).encode()
        )
