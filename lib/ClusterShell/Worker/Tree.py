...
class TreeWorker(DistantWorker):
    ...
    def __init__(self, nodes, handler, timeout, **kwargs):
        ...
        if self.command is None and self.source is None:
            raise ValueError(
                "missing command or source parameter in TreeWorker constructor"
            )
        ...
        python_executable = os.getenv(
            "CLUSTERSHELL_GW_PYTHON_EXECUTABLE", basename(sys.executable or "python")
        )
        ...
        # IO port
        self._port = EnginePort(handler=TreeWorker._IOPortHandler(self), autoclose=True)
    ...
    def _launch(self, nodes):
        self.logger.debug(
            "TreeWorker._launch on %s (fanout=%d)", nodes, self.task.info("fanout")
        )
        ...
        for gw, targets in next_hops:
            if gw == targets:
                self.logger.debug(
                    "task.shell cmd=%s source=%s nodes=%s timeout=%s remote=%s",
                    self.command,
                    self.source,
                    nodes,
                    self.timeout,
                ...
                self.logger.debug(
                    "added child worker %s count=%d", worker, len(self.workers)
                )
            else:
                self.logger.debug("trying gateway %s to reach %s", gw, targets)
                if self.source:
                    self._copy_remote(
                        self.source, destdir, targets, gw, self.timeout, self.reverse
                    )
                else:
    ...
    def _copy_remote(self, source, dest, targets, gateway, timeout, reverse):
        ...
        self.logger.debug(
            "_copy_remote gateway=%s source=%s dest=%s reverse=%s",
            gateway,
            source,
            dest,
            reverse,
    ...
    def _execute_remote(self, cmd, targets, gateway, timeout):
        ...
        self.logger.debug(
            "_execute_remote gateway=%s cmd=%s targets=%s", gateway, cmd, targets
        )
    ...
    def _relaunch(self, previous_gateway):
        ...
        self.logger.debug(
            "_relaunch on targets %s from previous gateway %s",
            targets,
            previous_gateway,
        )
        ...
        if self.eh is not None:
            self.eh._ev_routing(
                self,
                {"event": "reroute", "gateway": previous_gateway, "targets": targets},
            )
    ...
    def _on_remote_node_close(self, node, rc, gateway):
        ...
        self.logger.debug(
            "_on_remote_node_close %s %s via gw %s rc=%s",
            node,
            self._close_count,
            gateway,
            rc,
        ...
        if self.source and self.reverse:
            if node in self._rcopy_bufs:
                ...
                try:
                    self.logger.debug(
                        "%s extracting %d members in dest %s",
                        node,
                        len(tmptar.getmembers()),
                        self.dest,
                    )
    ...
    def _check_ini(self):
        self.logger.debug(
            "TreeWorker: _check_ini (%d, %d)", self._start_count, self._child_count
        )
    ...
    def _check_fini(self, gateway=None):
        ...
        if gateway:
            ...
            if not targets:
                ...
                self.logger.debug(
                    "TreeWorker._check_fini %s call pchannel_release for gw %s",
                    self,
                    gateway,
                )
    ...
    def _write_remote(self, buf):
        ...
        for gateway, targets in self.gwtargets.items():
            ...
            self.task._pchannel(gateway, self).write(
                nodes=targets, buf=buf, worker=self
            )
    ...
    def _gateway_abort(self, gateway):
        ...
        if gateway not in self.gwtargets:
            self.logger.warning("TreeWorker._gateway_abort %s not found", gateway)
            return
        targets = self.gwtargets[gateway]
        self.logger.debug(
            "TreeWorker._gateway_abort %s found: targets=%s", gateway, targets
        )
